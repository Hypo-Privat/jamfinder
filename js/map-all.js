	...
	$.getJSON('php/datafetcher.php?function=getEventsDetail&location_id=' + event, function(data) {
		$.each(data, function(counter, daten) {
			if (daten.location_id > 0) {
				...
				var detailString = '<dl id="c2"><dt>' + daten.t_date + ' ' + daten.t_kategorie + ' ' + daten.t_eventname + '</dt><dd> <a href="' + daten.url + '" target="_blank"><img src="' + defaultpic + '" style="width: 100px;height:100px "align="middle" /></a><br>' + daten.t_text + ' <br><p><b> Location: </b>' + daten.company + '  ' + daten.city + '  ' + daten.postcode + '  ' + daten.street + '<br>   ' + daten.comment + '</p></dd></dl>';
			...
			window.setTimeout(function() {
				...
				$.getJSON('php/datafetcher.php?function=getEvents&longitude=' + meineLongitude + '& latitude=' + meineLatitude + '&date=' + Date.now(), function(data) {
					...
					$.each(data, function(counter, daten) {
						...
						var contentString = '<h2>' + daten.t_date + ' ' + daten.t_kategorie + ' ' + daten.t_eventname + '</h2>  <b>' + daten.company + ' ' + daten.city + ' ' + daten.postcode + ' ' + daten.street + '  ' + daten.url + '</b><br><p>' + daten.t_text + '</p>';
